...
const MAX_CONCURRENT_PDF_PAGES = Number(process.env.PDF_MAX_CONCURRENCY || 2);
let _activePdfPages = 0;
const _pdfWaiters = [];

async function acquirePdfSlot() {
  if (_activePdfPages < MAX_CONCURRENT_PDF_PAGES) {
    _activePdfPages++;
    return;
  }
  await new Promise(resolve => _pdfWaiters.push(resolve));
  _activePdfPages++;
}

function releasePdfSlot() {
  _activePdfPages = Math.max(0, _activePdfPages - 1);
